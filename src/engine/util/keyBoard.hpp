...
struct KeyBoard_t {
	...
	using opIterKey = std::optional<std::unordered_map<KeySym, bool>::iterator>;

	// Evitar copiar o asignar teclado
	...
	bool isKeyPress(KeySym)  noexcept;
	void keyPressed(KeySym)  noexcept;
	void keyReleased(KeySym) noexcept;
	void reset() 	         noexcept;

private:

	opIterKey getKeyIterator(KeySym) noexcept;
	void setKey(KeySym, bool) noexcept;

	std::unordered_map<KeySym, bool> m_keys {
		  { XK_BackSpace       ,  false }	
		, { XK_Tab             ,  false }	
		, { XK_Linefeed        ,  false }	
		, { XK_Clear           ,  false }	
